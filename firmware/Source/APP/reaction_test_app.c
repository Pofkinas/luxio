...
#define START_STOP_BUTTON eButton_StartStop
    ...
    while (true) {
        ...
        switch (g_reaction_test_state) {
            case eReactionTestState_Init: {
                ...
                if (osEventFlagsWait(g_start_button_event, BUTTON_TRIGGERED_EVENT, osFlagsWaitAny, osWaitForever) != BUTTON_TRIGGERED_EVENT) {
...
bool Reaction_Test_App_Init (void) {
    ...
    if (!Button_API_Init(START_STOP_BUTTON, g_start_button_event)) {
        return false;
    }

    // TODO: Make VL53L0X API Init all
    ...
    for (eModule_t module = eModule_First; module < eModule_Last; module++) {
        ...
        g_dynamic_reaction_test_desc[module].led_strip_length = g_dynamic_reaction_test_desc[module].total_led_count * SINGLE_SEGMENT_LENGTH_MM;
